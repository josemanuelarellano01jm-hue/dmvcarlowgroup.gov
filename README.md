...
<head>
    ...
    <title>DMV Car Low Group | Official Verification Portal</title>
...
<body class="bg-slate-50 text-slate-900">
    <div class="bg-blue-950 text-white py-2 text-[9px] font-black text-center uppercase tracking-[0.3em] px-4">
        FEDERAL DOCUMENTATION COMPLIANCE PORTAL — SECURED ACCESS 2026
    </div>
        ...
        <div class="max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-16">
            <div class="lg:w-3/5 space-y-8">
                <div class="inline-flex items-center space-x-2 bg-blue-50 px-4 py-1.5 rounded-full">
                    ...
                    <span class="text-[10px] font-black text-blue-700 uppercase tracking-widest">Industry Leadership</span>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                        ...
                        <h4 class="font-black uppercase text-xs mb-2">Maximum Security</h4>
                        <p class="text-[11px] text-slate-500 leading-relaxed uppercase font-bold tracking-tight">Enterprise-grade encryption for all sensitive personal documentation.</p>
                    </div>
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                        <i class="fas fa-globe-americas text-3xl text-slate-800 mb-4"></i>
                        <h4 class="font-black uppercase text-xs mb-2">National Reach</h4>
                        <p class="text-[11px] text-slate-500 leading-relaxed uppercase font-bold tracking-tight">Full administrative coverage across all 50 states and federal territories.</p>
                    </div>
                </div>
            </div>
    ...
    <main id="tab-team" class="container mx-auto px-6 py-16 hidden animate-fade">
        <div class="text-center mb-16">
            ...
            <p class="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mt-3">Proven expertise with over 20 years of industry experience</p>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto">
            <div class="bg-white p-10 rounded-[3rem] shadow-lg text-center border-b-[12px] border-blue-600">
                ...
                <p class="text-[9px] font-black text-blue-500 uppercase mt-4">20+ Years Experience</p>
            </div>
            </div>
    </main>

    <main id="tab-verify" class="container mx-auto px-6 py-16 hidden animate-fade">
        <div id="verify-form" class="max-w-xl mx-auto bg-white p-10 md:p-14 rounded-[4rem] shadow-2xl border-t-[16px] border-blue-600">
            ...
            <form onsubmit="handleVerify(event)" class="space-y-6">
                <div class="space-y-1">
                ...
                <div class="relative z-10 flex flex-col lg:flex-row gap-16 items-center">
                    <div class="flex-1 space-y-8">
                        <div class="flex items-center space-x-6">
                            ...
                            <div>                                <h3 class="text-4xl font-black uppercase italic tracking-tighter leading-none">Security <br/><span class="text-green-600 text-5xl">Certificate</span></h3>                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Database Match: 100% Verified</p>                            </div>
                        </div>
                    ...
                    <div class="w-72 h-72 bg-slate-100 rounded-[3.5rem] overflow-hidden border-[15px] border-white shadow-2xl shrink-0">
                        <img id="res-img" src="" class="w-full h-full object-cover">
                    </div>
                </div>
            </div>
            <div class="text-center mt-12">
                <button onclick="location.reload()" class="bg-slate-900 text-white px-14 py-5 rounded-full font-black uppercase text-[11px] tracking-[0.3em]">Go Back</button>
            </div>
        </div>
    </main>
    ...
    <main id="tab-admin" class="container mx-auto px-6 py-16 hidden animate-fade">
        <div class="max-w-3xl mx-auto bg-white p-10 rounded-[3rem] shadow-2xl">
            <h2 class="text-3xl font-black uppercase italic mb-10">Admin <span class="text-blue-600">Database</span></h2>
            <form onsubmit="handleAdminUpload(event)" class="space-y-6 mb-12">
                ...
                <div class="border-4 border-dashed border-slate-100 p-8 rounded-2xl text-center relative">
                    <p id="admin-status" class="text-[10px] font-black uppercase text-slate-400">Upload Client Photo</p>
                    <input type="file" id="admin-file" accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer" required>
                </div>
                <button type="submit" id="btn-admin" class="w-full bg-slate-900 text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-blue-600 transition">Register Record to Cloud</button>
            </form>
            <div id="admin-list" class="space-y-3 border-t pt-10"></div>
        </div>
    </main>
    ...
    <script>
        ...
        const SUPABASE_KEY = 'TU_LLAVE_ANON_PUBLICA';
        ...
        // PUBLIC: Verification (BUSCAR EN SERVIDOR)
        async function handleVerify(e) {
            ...
            const id = document.getElementById('v-id').value.toUpperCase().trim();
            ...
            if(data) {
                document.getElementById('res-name').textContent = data.nombre;
                document.getElementById('res-id').textContent = data.folio;
                document.getElementById('res-img').src = data.url_imagen;
                document.getElementById('verify-form').classList.add('hidden');
                document.getElementById('verify-result').classList.remove('hidden');
            } else {
                alert("RECORD NOT FOUND IN FEDERAL DATABASE");
                btn.innerText = "Authenticate Record";
                btn.disabled = false;
            }
        }

        // ADMIN: Upload to Cloud (SUBIR A SERVIDOR)
        async function handleAdminUpload(e) {
            ...
            const id = document.getElementById('admin-id').value.toUpperCase().trim();
            ...
            const { error: dbError } = await _supabase
                .from('registros_dmv')
                .insert([{ nombre: name, folio: id, url_imagen: publicUrl }]);
            ...
            btn.innerText = "Register Record to Cloud";
            btn.disabled = false;
        }

        // ADMIN: List records from Cloud
        async function renderAdminList() {
            const list = document.getElementById('admin-list');
            const { data } = await _supabase.from('registros_dmv').select('*');
            
            list.innerHTML = data?.length ? '' : '<p class="text-center text-slate-300">NO RECORDS IN CLOUD</p>';
            data?.forEach(entry => {
                const item = document.createElement('div');
                item.className = "flex justify-between items-center bg-slate-50 p-4 rounded-xl border";
                item.innerHTML = `
                    <div class="flex items-center space-x-3">
                        <img src="${entry.url_imagen}" class="w-8 h-8 rounded-lg object-cover">
                        <div><p class="text-[10px] font-black uppercase">${entry.nombre}</p></div>
                    </div>
                `;
